  ...
  constructor(bufferSize = DEFAULT_BUFFER_SIZE) {
    ...
    _defineProperty(this, "addCallbackReturningId", callback => {
      try {
        ...
        this.addTelemetryEvent('Could not generate enough random numbers', getErrorStackTrace(error), 'RandomIdGenerationError'); // Not using InternalLog here as it is not yet instantiated
        ...
        try {
          ...
          if (item._type === 'RETURNING_ID') {
            try {
              ...
              InternalLog.log(`Error running a callback returning an id in Buffer: ${error}`, SdkVerbosity.WARN);
            }
          ...
          if (item._type === 'WITH_ID') {
            ...
            if (callbackId !== null && callbackId !== undefined) {
              ...
              InternalLog.log(`1 event was not sent as callback id was ${callbackId === null ? 'not set' : 'already unset'}`, SdkVerbosity.WARN);
            }
          ...
          InternalLog.log(`Error while draining Datadog Buffer: ${error}`, SdkVerbosity.WARN);
        }
      }
    ...
    _defineProperty(this, "drainTelemetry", () => {
      Object.values(this.telemetryBuffer).forEach(({
        ...
        DdSdk.telemetryError(`${message} happened ${occurences} times.`, stack, kind);
      });
    });
