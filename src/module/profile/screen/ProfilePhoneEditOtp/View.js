...
function ProfilePhoneEditOtp(props) {
  const {
    navigation,
    lang,
    colorScheme,
  ...
  const profileResult = useCallback(
    (act) => {
      if (act === SET_PHONE_NUMBER_SUCCESS) {
        ...
        setUserData({
          userData: {
            mobilePhoneNumber: params?.otpSendTo,
          },
        });
        setPhoneNumberClear();
      ...
      if (act === SET_PHONE_NUMBER_FAILED) {
        ...
        if (setPhoneNumberFailed?.message !== 'INTERNAL_SERVER_ERROR') {
          if (setPhoneNumberFailed?.message === 'WRONG_OTP') {
            setErrorMsg(true);
            return;
          }
          Alert.alert(setPhoneNumberFailed?.message);
        }
      }
  ...
  }, [isSubmit, otp, params?.otpSendTo, setLoading, setPhoneNumber]);
  ...
  function renderSuccessModal() {
    const text =
      params?.actionType === 'add'
        ? trans(locale, lang, 'nomorHpKamuBerhasilDitambahkan')
...
export default ProfilePhoneEditOtp;

ProfilePhoneEditOtp.propTypes = {
  ...
  setPhoneNumberClear: PropTypes.func.isRequired,
