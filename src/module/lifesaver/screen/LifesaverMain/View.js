...
import { ArrowDown2White, Kado } from 'ca-config/Svg';
...
import { setDobGoogle, setSignInSocial, useMount } from 'ca-util/common';
...
import ModalLoading from 'ca-component-modal/ModalLoading';
...
import {
  ...
  ModalNotKyc,
  ModalAgreement,
  ModalAgreement as ModalAgreementOther,
  ModalListRsV2,
...
function LifesaverMain(props) {
  ...
  const {
    LIFESAVER: { LIFESAVER_PLUS, LIFESAVER, LIFESAVER_POS },
  } = PRODUCT;
  ...
  const [isShowModalAgreementOther, setIsShowModalAgreementOther] =
    useState(false);
  ...
  useEffect(() => {
    if (isFetch && !getCurrentSubsFetch && !getEligiblePosFetch) {
      setIsLoad(false);
    }
  }, [getCurrentSubsFetch, getEligiblePosFetch, isFetch]);
  ...
  }, [getCurrentSubs, getEligiblePos, isFocused, userId]);
  ...
  const setShowModal = useCallback(
    (call) => {
      ...
      if (!call.alreadyKYC || !call.alreadySetPin) {
  ...
  const onBuyForOtherPress = () => {
    ...
    //   return setIsShowModalNotKyc(true);
    // }
    return setIsShowModalAgreementOther(true);
  };
  ...
  const renderTextSubscribe = useMemo(() => {
    if (
      (currentSubs === LIFESAVER && selectedPackage === LIFESAVER_PLUS) ||
      (currentSubs === LIFESAVER_POS &&
        (selectedPackage === LIFESAVER_PLUS || selectedPackage === LIFESAVER))
    ) {
  ...
  return (
    <View style={style.container}>
      ...
      <ScrollView
        ...
        <ScrollContents
          lang={lang}
          data={getTotalClaimResponse}
          onGetListRSPress={() => setListRs(true)}
        />
        <NeverGameOver {...props} />
        <PricesTable
          lang={lang}
        ...
        <ImageBackground
          ...
          <Button
            disabled={
              ...
              (currentSubs === LIFESAVER_POS &&
                selectedPackage === LIFESAVER_POS) ||
              (currentSubs === LIFESAVER && selectedPackage === LIFESAVER_POS)
            }
          ...
          {/* <Button
            onPress={onBuyForOtherPress}
            color={Color.whiteCard.light.color}
            borderColor={Color.buttonGradient.light.buttonGradient1}
            titleColor={Color.buttonGradient.light.buttonGradient1}
            style={style.buyForOther}
            suffixIcon={<Kado />}>
            {trans(locale, lang, 'beliProteksiUntuk')}
          </Button> */}
      ...
      />
      <ModalListRsV2 isVisible={listRs} onPressClose={() => setListRs(false)} />
      <ModalAgreementOther
        lang={lang}
        isVisible={isShowModalAgreementOther}
        onRequestClose={() => setIsShowModalAgreementOther(false)}
        onClosePress={() => setIsShowModalAgreementOther(false)}
        locale={locale}
        onPressTnc={() => {
          setIsShowModalAgreementOther(false);
          setIsComingFromScreen({
            screen: NAVIGATION.LIFESAVER.LifesaverMain,
            params: {
              isShowModal: true,
        ...
        onSubs={() => {
          ...
          navigation.navigate(NAVIGATION.LIFESAVER.LifesaverOrderOther, {
            product: selectedPackage,
            from: 'start',
            recurring: false,
          });
...
LifesaverMain.propTypes = {
  ...
  getTotalClaim: PropTypes.func.isRequired,
  getTotalClaimResponse: PropTypes.objectOf(Object).isRequired,
  getEligiblePosFetch: PropTypes.bool.isRequired,
};
