...
import { useDefaultBackHandler, useInterval } from 'ca-util/common';
...
function UpdataOtp(props) {
  ...
  useEffect(() => {
    setUpdataVerifyOtpResult(updataAction);
  }, [setUpdataVerifyOtpResult, updataAction]);

  const setUpdataVerifyOtpResult = useCallback(
    (act) => {
      ...
      if (act === SET_UPDATA_VERIFY_OTP_FAILED) {
        ...
        if (setUpdataVerifyOtpFailed?.message !== 'INTERNAL_SERVER_ERROR') {
          if (setUpdataVerifyOtpFailed?.message === 'WRONG_OTP') {
            setErrorMsg(true);
            return;
          }
          Alert.alert(setUpdataVerifyOtpFailed?.message);
        }
      }
      ...
      if (act === SET_UPDATA_REQUEST_OTP_FAILED) {
        ...
        if (setUpdataRequestOtpFailed?.message !== 'INTERNAL_SERVER_ERROR') {
          Alert.alert(setUpdataRequestOtpFailed?.message);
        }
      }
    },
    [
      ...
      <View
        style={[
          DeviceInfo.isTablet()
            ...
            : [
                style.input.line,
                { width: (48 * dimensions.width) / (375 + 40) - 4 },
              ],
...
UpdataOtp.propTypes = {
  ...
  setUpdataVerifyOtpFailed: PropTypes.objectOf(Object).isRequired,
  setUpdataVerifyOtp: PropTypes.func.isRequired,
  setUpdataVerifyOtpClear: PropTypes.func.isRequired,
